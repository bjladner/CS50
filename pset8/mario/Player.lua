...
function Player:update(dt)
    ...
    if self.dx < 0 then
        ...
        self.x = math.min(self.map.mapWidthPixels - self.width, self.x + self.dx * dt)
    end
    ...
    -- if you fall in a hole, you 'died'
    if self.y >= VIRTUAL_HEIGHT then
        gameState = 'died'
    end
    -- if you pass the flag, you completed the level
    if self.x >= self.map.flagXPixels and self.map:collides(self.map:tileAt(self.x, self.y + self.height)) then
        gameState = 'completed'
    end 
end
